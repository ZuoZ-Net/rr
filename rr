...
class Rrmj
{
    ...
    private const WEB_APPLET_REFERER = 'https://m.yichengwlkj.com/';
    private const WEB_APPLET_ORIGIN = 'https://m.yichengwlkj.com';
    private const WEB_APPLET_UA = 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.5 Mobile/15E148 Safari/604.1';
    private const WEB_APPLET_SEC_CH_UA = '"Chromium";v="146", "Not-A.Brand";v="24", "Google Chrome";v="146"';
    ...
    private function buildWebAppletRequestHeaders(array $baseHeaders, array $signedHeaders, string $umid, string $token): array
    {
        return array_merge($baseHeaders, $signedHeaders, [
            'User-Agent' => self::WEB_APPLET_UA,
            'Accept' => 'application/json, text/plain, */*',
            'Accept-Language' => 'zh-CN,zh;q=0.9',
            'Origin' => self::WEB_APPLET_ORIGIN,
            'deviceId' => $umid,
            'umid' => $umid,
            'uet' => '9',
            'token' => $token,
            ...
            'Referer' => self::WEB_APPLET_REFERER,
            'sec-ch-ua' => self::WEB_APPLET_SEC_CH_UA,
            'sec-ch-ua-mobile' => '?1',
    ...
    private function get_request_data_by_ids(string $dramaId, string $episodeSid, string $qualityCode = 'AI_OD', $umid = '', $token = ''): array
    {
        ...
        $method = 'GET';
        $param_s = '/m-station/drama/play?hsdrOpen=0&dramaId='.$dramaId.'&episodeSid='.$episodeSid.'&quality='.$qualityCode.'&hevcOpen=1&tria4k=0';
        ...
        $headers = $this->buildWebAppletRequestHeaders($baseHeaders, $headers_with_sign, $umid, $token);
    ...
    private function get_page_data($dramaId, $umid = '', $token = '', bool $retried = false): ?array
    {
        ...
        $reqHeaders = $this->buildWebAppletRequestHeaders($baseHeaders, $sign, $umid, $token);
